...
class greenhouseParameters
  "A central location for all the building parameters, referenced by other components"
      ...
      parameter Real outside_albedo_NIR = 0.8 "this is NIR albedo of the field around the greenhouse";
    ...
    //glazing, Panel initialization
      ...
      parameter Integer n_panels = 2 "number of flat panels, THIS NEEDS TO BE A CONSTANT somewhere in the greenhouseModel so lightAbsorbtion can refer to that for a constant n_Panels value. ";
      parameter Modelica.Units.SI.Area panel_areas[:] = { 2,  2} "panel areas";
      parameter Modelica.Units.SI.Angle panel_azimuths[:] = {  0,   0};
      ...
      parameter  Modelica.Units.SI.Angle panel_zeniths[ :] = {   0, Modelica.Units.Conversions.from_deg(90)}
       "list of panel zeniths in radians, as measured from panel normal vector,
        The angle between a line perpendicular to the earth's surface and the panel normal.
         (90 deg = vertical wall ; 0 deg = flat horizontal panel)";
     ...
     parameter Real glazing_FIR_transmission = 0.11 "This is thermal radiation transmission, 1 - thermicity,
     ...
     parameter Real haze=0.27 "This is fraction of direct light incident that is diffused. ";
     ...
     parameter Modelica.Units.SI.HeatFlowRate heater_max_power = 40 "max watts for heater";
     ...
     parameter Modelica.Units.SI.Volume water_volume_M3_per_M2 = 1 "volume of water for each m2 of floor growing area";
     parameter Real water_Rsi_max = 20/5.67 "max R for when water is thermally uncoupled from air, assume area is equal to floor area";
     parameter Real water_Rsi_min = 0.001/5.67 "min R for when water is thermally coupled to air, assume area is equal to floor area";
     parameter Modelica.Units.SI.Temperature water_recharge_temp = Modelica.Units.Conversions.from_degC(25) "temperature at which air startes to heat the water";
     parameter Modelica.Units.SI.Temperature water_discharge_temp = Modelica.Units.Conversions.from_degC(16) "temperature at which air startes to be heated by the water";
